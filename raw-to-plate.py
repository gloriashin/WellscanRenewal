...
filenames = os.listdir('/media/seungyun/8TBHardDisk/scanner/20241024_reduced/')
filename_base = filename_tif.replace('.jpeg','').replace('.jpg','')

    #filename_tif = '/work/taejoon/Dropbox/Public/pub/XenopusIn96_2014feb04_day3.tif'
for filename_tif in filenames:
    ...
    im.save('20241024_plate/%s.rotate.jpg'%filename_base,'jpeg')
    x_max, y_max = im.size
    im_array = np.array(im) ## transpose x&y
    ...
    im_draw.save('20241024_plate/%s.crop_corner.jpg'%filename_base,'jpeg')
    ...
    im.save('20241024_plate/%s.flip_grid.jpg'%filename_base,'jpeg')
    sys.exit(1)
    ...
    for col in range(0,12):
        for row in range(0,8):
            ...
            im_well.save('20241024_plate/%s_%s%02d.tif'%(filename_base,row_list[row],col+1),'tiff')
    #plt.gray()
    #plt.imshow(bw_array)
